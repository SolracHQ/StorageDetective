...
proc treeDisplayer(x: TreeItem): string =
  ...
  case x.kind
  of tkFile:
    result = &"{x.file.size%x.file.parent.size:>5}% {x.file.size:>10} {x.file.name}"
  of tkDir:
    result = &"{x.dir.size%x.dir.parent.size:>5}% {x.dir.size:>10} {x.dir.name}"
  of tkUpLink:
    result = &"{x.parent.size:>17} .."
...
proc analyzingDialog(path: string, terminalBuffer: var TerminalBuffer) =
  ...
  terminalBuffer.write(
    terminalBuffer.width div 2 - message.len div 2, terminalBuffer.height div 2, message
  )

  # Draw the current path
  let displayPath =
    if path.len > terminalBuffer.width - 10:
      shortenPath(path, terminalBuffer.width - 10)
    else:
      path
  terminalBuffer.write(
    terminalBuffer.width div 2 - displayPath.len div 2,
    terminalBuffer.height div 2 + 1,
    displayPath,
  )
...
proc main() =
  ...
  var terminalBuffer: TerminalBuffer =
    newTerminalBuffer(terminalWidth(), terminalHeight())
  ...
  while true:
    ...
    menu.draw(
      (1, 1, terminalBuffer.width - 2, terminalBuffer.height - 1), terminalBuffer
    )
    ...
    of Key.Enter:
      if menu.selected.isSome and menu.selected.get.kind == tkDir:
        ...
        menu = newMenu(dir.items, treeDisplayer)
    of Key.Up, Key.Left, Key.K:
      menu.dec
    of Key.Down, Key.Right, Key.J:
      menu.inc
    of Key.G:
      cfg.grouping.toggle()
      menu = newMenu(dir.items, treeDisplayer)
